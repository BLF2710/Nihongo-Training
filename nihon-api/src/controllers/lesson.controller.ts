...
import { LESSONS, canUserAccessLesson } from "../services/lesson.service";
...
export async function listLessons(req: Request, res: Response) {
  try {
    ...
    const lessons = await Promise.all(LESSONS.map(async (lesson) => ({ ...lesson, ...(await canUserAccessLesson(userId, lesson)) })));
...
export async function getLessonAccess(req: Request, res: Response) {
  ...
  const access = await canUserAccessLesson(currentUserId(req), lesson);
...
export async function completeLesson(req: Request, res: Response) {
  try {
    ...
    if (!lesson) return res.status(404).json({ message: "Lesson not found" });
    const access = await canUserAccessLesson(userId, lesson); if (!access.allowed) return res.status(403).json({ message: "Lesson is locked", reasons: access.reasons });
